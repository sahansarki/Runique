...
class TrackerViewModel(
    ...
    init {
            ...
            .onEach { isTracking ->
                ...
                if (result is Result.Error) {
                    result.error.toUiText()?.let {
                        eventChannel.trySend(TrackerEvent.Error(it))
                    }
                }
        ...
        viewModelScope.launch {
            val hearRateSupported = exerciseTracker.isHeartRateTrackingSupported()
            state = state.copy(canTrackHeartRate = hearRateSupported)
        }
    ...
    fun onAction(action: TrackerAction, triggeredOnPhone: Boolean) {
        if (!triggeredOnPhone) {
